...
proc checkDB(
  ...
  let selectStmt = db.backend
    .prepareStmt(
      "SELECT * FROM metadata;", NoParams, (int64, Hash32), managed = false
...
proc toSPDIR*(db: SlashingProtectionDB_v2): SPDIR {.raises: [IOError].} =
  ...
  block:
    let selectRootStmt = db.backend
      .prepareStmt(
        "SELECT genesis_validators_root FROM metadata;", NoParams, Hash32, managed =
          false # manual memory management
      )
      .get()
  ...
  block:
    let selectValStmt = db.backend
      .prepareStmt(
        "SELECT public_key FROM validators;", NoParams, PubKeyBytes, managed = false
          # manual memory management
      )
      .get()
