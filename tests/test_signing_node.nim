...
proc spawnSigningNodeProcess(
  ...
  let arguments =
    ...
    of RemoteSignerType.Web3Signer:
      @[
        "--non-interactive=true", "--log-level=DEBUG",
        "--data-dir=" & getTestDir(rt) & "/signing-node",
        "--bind-address=" & SigningNodeAddress,
        "--bind-port=" & $getNodePort(basePort, rt), "--request-timeout=" &
          $SigningRequestTimeoutSeconds,
    ...
    of RemoteSignerType.VerifyingWeb3Signer:
      @[
        ...
        "--expected-fee-recipient=" & $SigningExpectedFeeRecipient, "--request-timeout=" &
          $SigningRequestTimeoutSeconds,
          # we make so low `timeout` to test connection pool.
      ]
